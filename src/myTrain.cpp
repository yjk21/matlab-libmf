...
namespace
{
    ...
    std::shared_ptr<GriddedMatrix> read_gridded_matrix(
            ...
            const Matrix & Tr_meta,
            const arma::Col<int> & I,
            const arma::Col<int> & J,
            ...
            TrainOption const &option)
    {
        ...
        for(int tx = 0; tx < option.nr_threads; tx++)
        {
            ...
            if(err)
                ::mexErrMsgTxt("creating new thread failed");
...
void getParams(libMFParams & params, const mxArray * prhs[]){
    ...
    int nThreads = int(::mxGetScalar(prhs[16]));
    ...
    params.lambdaq = lambdaq;
    params.eta = eta;
    params.ub = useub;
    params.ib = useib;
    params.avg = useavg;
    params.nThreads = nThreads <= 0 ? nThreads : 1;
    params.M = M;
    params.N = N;
}
...
void params2vec(const libMFParams & params, vector<string> & argAll){
    ...
    string sLambdap = std::to_string(params.lambdap);
    string sLambdaq = std::to_string(params.lambdaq);

    argAll.push_back("-k"); 
    argAll.push_back( std::to_string(params.D)); 
    argAll.push_back("-t"); 
    argAll.push_back( std::to_string(params.maxIt)); 
    argAll.push_back("-s"); 
    argAll.push_back( std::to_string(params.nThreads)); 
    argAll.push_back("-g"); 
    argAll.push_back( std::to_string(params.eta)); 
    argAll.push_back("-q"); 
    argAll.push_back( sLambdaq); 
    argAll.push_back("-p"); 
    argAll.push_back( sLambdap); 
    argAll.push_back("-ub"); 
    if(params.ub)
        argAll.push_back( sLambdap ); 
    else
    ...
    if(params.ib)
        ...
        argAll.push_back( "-1" ); 
    if(params.avg)
        argAll.push_back( "--use-avg" ); 
    else
        argAll.push_back( "--no-use-avg" ); 

    argAll.push_back("trainpath");
}
void train_help()
{
    printf(
...
std::shared_ptr<TrainOption> initLibMF(const libMFParams & params ){
    vector<string> argAll;
    //argAll.push_back("libmf");
    //argAll.push_back("libmf");

    params2vec(params, argAll);

    char **  args = new char*[argAll.size()] ;

    for(unsigned int it = 0; it < argAll.size(); it++){
        const string & argStr = argAll[it];
        args[it] = new char[argStr.size()+1 ];
        argStr.copy(args[it], argStr.size());
        args[it][argStr.size()] = '\0';
    }
    std::shared_ptr<TrainOption> r = parse_train_option(argAll.size(), args);

    for(unsigned int it = 0; it < argAll.size(); it++){
        cout << string(args[it]) <<endl;
        delete[] args[it];
    }
    delete[] args;
    return r;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    const arma::Col<int> Itr(ItrPr, nTr, false, true); //wrap pointers
    const arma::Col<int> Jtr(JtrPr, nTr, false, true); // in vectors
    const arma::Col<float> Vtr(VtrPr, nTr, false, true); //training

    const arma::Col<int> Ite(ItePr, nTe, false, true); //wrap poitners
    const arma::Col<int> Jte(JtePr, nTe, false, true); // in vectors
    const arma::Col<float> Vte(VtePr, nTe, false, true); //test

    libMFParams params;
    getParams(params, prhs);

    if (params.M - 1 < arma::max(Itr) || params.M - 1 < arma::max(Ite) 
            || params.N - 1 < arma::max(Jtr) || params.N - 1 < arma::max(Jte)) {
        ::mexErrMsgTxt("User or item index out of bounds!\n");
    }

    std::shared_ptr<const TrainOption> const
        option = initLibMF(params);

    float mu =params.avg ? arma::mean(Vtr) : 0.0; 
    cout << "1"<< endl;
    std::shared_ptr<Matrix> Tr_meta(new Matrix);
    Tr_meta->nr_users = params.N;
    Tr_meta->nr_items = params.M;
    Tr_meta->nr_ratings = nTr;
    Tr_meta->avg = mu;
    ...
    std::shared_ptr<const GriddedMatrix> const
        Tr = read_gridded_matrix(*option, user_map, item_map, *Tr_meta, Itr, Jtr, Vtr);
    timer.toc("done.");

    std::shared_ptr<Matrix> Va(new Matrix);
    timer.reset("Reading validation data...");
    //Va = read_matrix(option->va_path);

    Va->nr_users = params.N;
    Va->nr_items = params.M;
    Va->nr_ratings = nTe;
    Va->avg = mu;
    ...
    initOutputPr(0, plhs, params.D, params.M,  qPr);
    initOutputPr(1, plhs, params.D, params.N,  pPr);
    initOutputPr(2, plhs, 1, params.M,  ibPr);
    initOutputPr(3, plhs, 1, params.N,  ubPr);
    initOutputPr(4, plhs, 1, 1,  avgPr);

    arma::Mat<float> Qout(qPr, params.D, params.M, false, true);
    arma::Mat<float> Pout(pPr, params.D, params.N, false, true);

    int const dim_aligned = get_aligned_dim(model.param.dim);
    arma::Mat<float> Qm(model.Q, dim_aligned, params.M, false, true);
    arma::Mat<float> Pm(model.P, dim_aligned, params.N, false, true);

    //Copy factors
    Qout = Qm.rows(0, params.D - 1);
    Pout = Pm.rows(0, params.D - 1);

    //Copy bias terms
    if (model.IB.size() == params.M) {
        std::copy(model.IB.begin(), model.IB.end() , ibPr);
    }

    if (model.UB.size() == params.N) {
        std::copy(model.UB.begin(), model.UB.end() , ibPr);
    }
